...
namespace nvhttp {
  ...
  /**
   * @brief Register a pending private-protocol auto-pair request.
   *
   * The entry is consumed only by a /pair?phrase=getservercert request whose
   * uniqueid and smp_request_id match this record.
   *
   * @param request_id Private handshake request id.
   * @param sink_unique_id Moonlight GameStream uniqueid expected on /pair.
   * @param pin Four-digit GameStream pairing PIN.
   * @param expires_at_unix_seconds Expiration timestamp in Unix seconds.
   * @return true when the pending entry was accepted.
   */
