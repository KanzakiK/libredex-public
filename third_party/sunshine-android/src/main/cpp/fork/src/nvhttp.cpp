...
namespace nvhttp {
  ...
  /**
   * @brief Pending private-protocol auto-pair entry.
   */
  struct smp_auto_pair_t {
    std::string request_id;
    std::string sink_unique_id;
    std::string pin;
    std::int64_t expires_at_unix_seconds = 0;
  };

  std::unordered_map<std::string, smp_auto_pair_t> map_smp_auto_pair;  ///< Pending SMP auto-pair entries keyed by request id.

  /**
   * @brief Pending manually submitted PIN, used when the UI/debug caller submits before Moonlight's request arrives.
   */
  struct pending_manual_pin_t {
    std::string pin;
    std::string name;
    std::int64_t expires_at_unix_seconds = 0;
  };

  std::optional<pending_manual_pin_t> pending_manual_pin;  ///< Manual PIN queued for the next pairing request.
  ...
  bool is_four_digit_pin(std::string_view pin) {
    return pin.size() == 4 && std::all_of(pin.begin(), pin.end(), [](unsigned char ch) {
      return std::isdigit(ch) != 0;
    });
  }

  void prune_expired_smp_auto_pairs() {
    const auto now = unix_seconds_now();
    for (auto it = map_smp_auto_pair.begin(); it != map_smp_auto_pair.end();) {
      if (it->second.expires_at_unix_seconds <= now) {
        it = map_smp_auto_pair.erase(it);
      } else {
        ++it;
      }
    }
  ...
  void prune_expired_manual_pin() {
    if (pending_manual_pin && pending_manual_pin->expires_at_unix_seconds <= unix_seconds_now()) {
      BOOST_LOG(info) << "Discarding expired queued Moonlight PIN"sv;
      pending_manual_pin.reset();
    }
  }

  bool register_pending_auto_pair(
    std::string request_id,
    std::string sink_unique_id,
    std::string pin,
    std::int64_t expires_at_unix_seconds
  ) {
    prune_expired_smp_auto_pairs();

    if (request_id.empty() || sink_unique_id.empty() || !is_four_digit_pin(pin)) {
      return false;
    }
    if (expires_at_unix_seconds <= unix_seconds_now()) {
      return false;
    }

    smp_auto_pair_t entry;
    entry.request_id = std::move(request_id);
    entry.sink_unique_id = std::move(sink_unique_id);
    entry.pin = std::move(pin);
    entry.expires_at_unix_seconds = expires_at_unix_seconds;
    map_smp_auto_pair[entry.request_id] = std::move(entry);
    return true;
  }

  void queue_manual_pin(std::string pin, std::string name) {
    pending_manual_pin = pending_manual_pin_t {
      std::move(pin),
      name.empty() ? "Moonlight"s : std::move(name),
      unix_seconds_now() + 60
    };
    BOOST_LOG(info) << "Queued Moonlight PIN for next pairing request"sv;
  }
  ...
  bool try_smp_auto_getservercert(pair_session_t &sess, const args_t &args, pt::ptree &tree) {
    auto request_id_it = args.find("smp_request_id"s);
    if (request_id_it == std::end(args)) {
      return false;
    }

    prune_expired_smp_auto_pairs();
    auto pending_it = map_smp_auto_pair.find(request_id_it->second);
    if (pending_it == map_smp_auto_pair.end()) {
      fail_pair(sess, tree, "Invalid private pairing request");
      return true;
    }

    auto pending = pending_it->second;
    map_smp_auto_pair.erase(pending_it);

    if (pending.sink_unique_id != sess.client.uniqueID) {
      fail_pair(sess, tree, "Private pairing uniqueid mismatch");
      return true;
    }
    if (pending.expires_at_unix_seconds <= unix_seconds_now()) {
      fail_pair(sess, tree, "Private pairing request expired");
      return true;
    }
    if (!is_four_digit_pin(pending.pin)) {
      fail_pair(sess, tree, "Invalid private pairing PIN");
      return true;
    }

    sess.client.name = "Moonlight";
    getservercert(sess, tree, pending.pin);
    BOOST_LOG(info) << "Accepted private Moonlight pairing request "sv << pending.request_id;
    return true;
  }

  bool try_manual_queued_getservercert(pair_session_t &sess, pt::ptree &tree) {
    prune_expired_manual_pin();
    if (!pending_manual_pin) {
      return false;
    }

    auto pending = std::move(*pending_manual_pin);
    pending_manual_pin.reset();
    if (!is_four_digit_pin(pending.pin)) {
      fail_pair(sess, tree, "Invalid queued PIN");
      return true;
    }

    sess.client.name = pending.name.empty() ? "Moonlight"s : std::move(pending.name);
    getservercert(sess, tree, pending.pin);
    BOOST_LOG(info) << "Accepted queued Moonlight PIN for uniqueid "sv << sess.client.uniqueID;
    return true;
  }
  ...
  void pair(std::shared_ptr<safe::queue_t<crypto::x509_t>> &add_cert, std::shared_ptr<typename SimpleWeb::ServerBase<T>::Response> response, std::shared_ptr<typename SimpleWeb::ServerBase<T>::Request> request) {
    ...
    if (it = args.find("phrase"); it != std::end(args)) {
      if (it->second == "getservercert"sv) {
        ...
        if (try_smp_auto_getservercert(ptr->second, args, tree)) {
          return;
        }
        if (try_manual_queued_getservercert(ptr->second, tree)) {
          return;
        }
  ...
  bool pin(std::string pin, std::string name) {
    ...
    if (!is_four_digit_pin(pin)) {
      tree.put("root.paired", 0);
      tree.put("root.<xmlattr>.status_code", 400);
      tree.put("root.<xmlattr>.status_message", "Pin must be 4 numeric digits");
      BOOST_LOG(warning) << "Rejected invalid Moonlight PIN length="sv << pin.size();
      return false;
    }

    prune_expired_manual_pin();
    if (map_id_sess.empty()) {
      queue_manual_pin(std::move(pin), std::move(name));
      return true;
    }

    auto has_async_response = [](auto &async_response) {
      return (async_response.has_left() && async_response.left()) ||
             (async_response.has_right() && async_response.right());
    };
    auto sess_it = std::find_if(std::begin(map_id_sess), std::end(map_id_sess), [&](auto &entry) {
      return entry.second.last_phase == PAIR_PHASE::NONE &&
             has_async_response(entry.second.async_insert_pin.response);
    });
    if (sess_it == std::end(map_id_sess)) {
      BOOST_LOG(warning) << "No pending Moonlight pairing request can accept a PIN"sv;
      return false;
    }

    auto &sess = sess_it->second;
    getservercert(sess, tree, pin);
    sess.client.name = name.empty() ? "Moonlight"s : std::move(name);
    ...
    if (async_response.has_left() && async_response.left()) {
      ...
      BOOST_LOG(warning) << "Moonlight PIN was accepted but no async response is available"sv;
      return false;
    }

    // reset async_response
    async_response = std::decay_t<decltype(async_response.left())>();
    BOOST_LOG(info) << "Submitted Moonlight PIN for uniqueid "sv << sess.client.uniqueID;
