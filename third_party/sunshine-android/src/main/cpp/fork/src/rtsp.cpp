...
#pragma pack(pop)
  ...
  public:
    ...
    void session_raise(std::shared_ptr<launch_session_t> launch_session) {
      if (auto discarded = launch_event.try_pop()) {
        BOOST_LOG(warning) << "Replacing stale pending RTSP launch session: "sv << discarded->unique_id;
        raised_timer.cancel();
      }
    ...
    void clear(bool all = true) {
      if (all) {
        raised_timer.cancel();
        if (auto discarded = launch_event.try_pop()) {
          BOOST_LOG(debug) << "Cleared pending RTSP launch session: "sv << discarded->unique_id;
        }
      }
  ...
  void cmd_announce(rtsp_server_t *server, tcp::socket &sock, launch_session_t &session, msg_t &&req) {
    ...
    if (config.monitor.videoFormat != 0 && config.monitor.videoFormat != 1) {
      BOOST_LOG(warning) << "Unsupported video codec requested: "sv << config.monitor.videoFormat;
    ...
    if (stream::session::start(
          *stream_session,
          net::addr_to_normalized_string(sock.remote_endpoint().address())
        )) {
