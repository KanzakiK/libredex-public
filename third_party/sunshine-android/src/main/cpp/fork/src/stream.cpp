...
#pragma pack(pop)
  ...
  /**
   * @brief Tell Moonlight that the stream is ending before closing the control peer.
   *
   * @param session Active streaming session.
   * @param server Control server used to send reliable control packets.
   * @return 0 when the termination message was sent; nonzero otherwise.
   */
  int send_termination(session_t *session, control_server_t *server) {
    if (!session->control.peer) {
      return -1;
    }
...

      {
        ...
        KITTY_WHILE_LOOP(auto pos = std::begin(*server->_sessions), pos != std::end(*server->_sessions), {
          ...
          if (session->state.load(std::memory_order_acquire) == session::state_e::STOPPING) {
            ...
            if (session->control.peer) {
              send_termination(session, server);
  ...
  void videoThread(session_t *session) {
    ...
    // Some Android Moonlight clients can spend almost the full regular ping
    // timeout finishing RTSP before they start the video UDP stream. Keep the
    // active-session timeout unchanged, but leave a small startup margin so the
    // host does not tear down the session just before PLAY and the first ping.
  ...
  namespace session {
    ...
    void join(session_t &session) {
      ...
      // A session can be stopped while its audio/video workers are still waiting for
      // their initial ping. Give those waits time to expire before treating the join
      // as hung. Using the same 10-second deadline as ping_timeout races the normal
      // timeout path and can abort the host just as the worker is exiting.
      auto force_kill = task_pool.pushDelayed(task, config::stream.ping_timeout + 10s).task_id;
    ...
    int start(session_t &session, const std::string &addr_string) {
      ...
      if (++running_sessions == 1) {
        platf::streaming_will_start(addr_string);
#if defined SUNSHINE_TRAY && SUNSHINE_TRAY >= 1
